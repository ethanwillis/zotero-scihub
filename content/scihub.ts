...
class ItemObserver {
  ...
  public async notify(event: string, type, ids: [number], extraData) {
...
class CScihub {
  ...
  private DEFAULT_SCIHUB_URL: string = 'https://sci-hub.tf/'
  private DEFAULT_AUTOMATIC_PDF_DOWNLOAD: boolean = true
  ...
  public async updateItems(items: [IZotero['Item']]) {
  ...
  private async updateItem(scihubUrl: URL, item: IZotero['Item']) {
  ...
  private getDoi(item: IZotero['Item']): string | null {
  ...
  private getDoiFromExtra(item: IZotero['Item']): string | null {
  ...
  private getDoiFromUrl(item: IZotero['Item']): string | null {
  ...
  private generateScihubItemUrl(item: IZotero['Item']): URL | null {
...
const Scihub = new CScihub() // tslint:disable-line:variable-name

window.addEventListener('load', event => {
  Zotero.Scihub.load().catch(err => Zotero.logError(err))
}, false)
window.addEventListener('unload', event => {
  Zotero.Scihub.unload().catch(err => Zotero.logError(err))
}, false)
